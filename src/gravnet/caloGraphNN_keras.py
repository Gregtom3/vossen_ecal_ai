...
class GravNet(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        for i in range(len(self.message_passing_layers)):
            ...
            d_weight = self.add_weight(
                name=self.subname + "_mp_distance_weight_" + str(i),
                shape=(1,),
                initializer="uniform",
...
class GarNet(tf.keras.layers.Layer):
    ...
    def call(self, x):
        ...
        aggregated_max = self.apply_edge_weights(
            features, edge_weights_trans, aggregation=tf.reduce_max
        )  # (B, S, F+S)
        aggregated_mean = self.apply_edge_weights(
            features, edge_weights_trans, aggregation=tf.reduce_mean
        )  # (B, S, F+S)
        ...
        updated_features = tf.concat(
            [x, updated_features, edge_weights], axis=-1
        )  # (B, V, X+2*S*(F+S)+S)
