...
def indexing_tensor(spatial_features, k=10, n_batch=-1):
    ...
    batch_range = tf.expand_dims(
        tf.expand_dims(tf.expand_dims(tf.range(0, n_batch), axis=1), axis=1), axis=1
    )
...
def high_dim_dense(inputs, nodes, **kwargs):
    if len(inputs.shape) == 3:
        return tf.layers.conv1d(
            inputs, nodes, kernel_size=(1), strides=(1), padding="valid", **kwargs
        )

    if len(inputs.shape) == 4:
        return tf.layers.conv2d(
            inputs, nodes, kernel_size=(1, 1), strides=(1, 1), padding="valid", **kwargs
        )

    if len(inputs.shape) == 5:
        return tf.layers.conv3d(
            inputs, nodes, kernel_size=(1, 1, 1), strides=(1, 1, 1), padding="valid", **kwargs
        )
...
def layer_GarNet(vertices_in, n_aggregators, n_filters, n_propagate, plus_mean=True):
    ...
    agg_nodes = tf.layers.dense(
        vertices_in_orig, n_aggregators, activation=None
    )  # BxVxNA, vertices_in: BxVxF
    ...
    vertices_in_collapsed = apply_edges(
        vertices_in, edges, reduce_sum=True, flatten=True
    )  # ,aggregation_function=tf.reduce_mean)# [BxNAxF]
    vertices_in_mean_collapsed = apply_edges(
        vertices_in, edges, reduce_sum=True, flatten=True, aggregation_function=tf.reduce_mean
    )  # [BxNAxF]
    ...
    expanded_collapsed = apply_edges(
        vertices_in_collapsed, edges, reduce_sum=False, flatten=True
    )  # [BxVxF]
...
def layer_GravNet(vertices_in, n_neighbours, n_dimensions, n_filters, n_propagate):
    ...
    def collapse_to_vertex(indexing, distance, vertices):
        ...
        distance = (
            distance * 10.0
        )  # input is tanh activated or batch normed, allow for some more spread
